  ...
  def main(args: Array[String]): Unit = {
    ...
    source.filter(_.contains("order_detail"))
      .map(data => {
        ...
        val product_id = nObject1.getString("product_id")
        val product_price = nObject1.getString("product_price").toDouble
        val product_cnt = nObject1.getString("product_cnt").toInt
        (product_id,product_cnt*product_price)
      }).keyBy(_._1)
      .sum(1)
      .keyBy(_ => true)
      ...
      }).map(data => ("top3itemconsumption",data))
